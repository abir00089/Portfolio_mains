...
// ─── Entry animation via pure Three.js lerp ──────────────────────────────────
// Previously used useMotionValue + useSpring from motion/react which created a
// *second* parallel RAF loop. Now we lerp entirely inside the R3F useFrame tick.
const START_Y  = 5;    // initial y position (off-screen high)
const TARGET_Y = -1;   // final resting y position
const LERP_SPEED = 4;  // damping coefficient (higher = faster settle)
...
export function Astronaut(props) {
  ...
  // Track current y with a ref — no state, no re-renders
  const currentY = useRef(START_Y);

  // ─── Single RAF tick: lerp entry animation + nothing else ───────────────
  ...
  return (
    <group
      ...
      position={props.position || [1.3, START_Y, 0]}
    >
